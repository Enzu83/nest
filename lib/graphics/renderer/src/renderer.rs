...
const DEFAULT_WINDOW_REFRESH_RATE: f32 = 60.0;
...
impl Renderer {
    pub fn default(event_loop: &EventLoop<()>) -> Result<Self> {
    ...
    pub fn fill(&mut self, color: &Color) {
        self.draw_rect(0, 0, self.pixels.texture().width(), self.pixels.texture().height(), color).unwrap()
    }
